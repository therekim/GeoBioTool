# geobiotool_rugosity_algorithm.py
# -*- coding: utf-8 -*-
import pandas as pd
import numpy as np
...
class GeoBioToolRugosityAlgorithm(QgsProcessingAlgorithm):

    def name(self) -> str:
        return "compute_rugosity_index_dustan_2013"

    def displayName(self) -> str:
        return "Compute Rugosity Index (Dustan et al. 2013)"
    ...
    def tags(self) -> list[str]:
        return ["rugosity", "surface complexity", "Dustan", "LiDAR"]
    ...
    def processAlgorithm(
        ...
        df_out = df.groupby(["yg", "xg"]).apply(rugosity).reset_index(name="rugosity_index")
        df_out.to_csv(out_csv, index=False, encoding="utf-8")
        feedback.pushInfo(f"Saved CSV: {out_csv}")

        if not df_out.empty:
            pivot = df_out.pivot(index="yg", columns="xg", values="rugosity_index")
            ...
            fig.colorbar(mesh, ax=ax, label="Rugosity Index")
            ax.set_aspect("equal")
            ax.set_xlabel("X")
            ax.set_ylabel("Y")
            fig.tight_layout()
    ...
    def createInstance(self):
        return GeoBioToolRugosityAlgorithm()
